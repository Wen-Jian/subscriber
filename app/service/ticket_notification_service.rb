module TicketNotificationService
    ...
    class TicketNotificationService
        def self.price_check
            ...
            htmlbody = ""
            settings.each do |s|
                # @tickets = FlightTicket.where("price < ?", s.price).where(destination: s.destination, flight_type: s.flight_type).order(flight_date: :asc)
                @tickets = FlightTicket.where("price < ?", 1000)
                if @tickets.present?
                    if s.flight_type == ConstantDefinition::DIRECT_FLIGT
                        @flight_type = '直飛'
                    else
                        @flight_type = '轉機'
                    end
